...
public interface HourlyTaskDataRestRepository extends CrudRepository<AdminHourlyTaskEntity, Integer> {
	...
	@Query("SELECT i FROM AdminHourlyTaskEntity i  ORDER BY i.id DESC")
	//@Query("SELECT i FROM AdminHourlyTaskEntity i WHERE i.deleted = 0 ORDER BY i.id DESC")
	@Cacheable
	public List<AdminHourlyTaskEntity> findAll();
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "allpagewise", rel = "allpagewise")
	@Query("SELECT i FROM AdminHourlyTaskEntity i  ORDER BY i.id desc")
	//@Query("SELECT i FROM AdminHourlyTaskEntity i WHERE i.deleted = 0 ORDER BY i.id desc")
	@Cacheable
	public Page<AdminHourlyTaskEntity> findAll(Pageable pageable);

	@Override
	//@PreAuthorize("hasAuthority('role_admin')")
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@CachePut
	<S extends AdminHourlyTaskEntity> S save(S hourlyTaskEntity);
...
//			"(:taskCompletedFrom is null or i.taskCompleted >= :taskCompletedFrom) AND (:taskCompletedTo is null or i.taskCompleted <= :taskCompletedTo) AND (:dateReceivedFrom is null or i.dateReceived >= :dateReceivedFrom) AND (:dateReceivedTo is null or i.dateReceived >= :dateReceivedTo) AND (:createdBy is null or i.createdBy.id = :createdBy)")
	@Query(value="SELECT i FROM AdminHourlyTaskEntity i WHERE" +
			...
			"(:createdBy is null or i.createdBy.id = :createdBy) ORDER BY CASE" +
			...
			" WHEN :columnName  = 'taskCompleted' THEN i.taskCompleted ELSE i.id END " +
			" END ASC, i.id desc ")
	public Page<AdminHourlyTaskEntity> findByQueryAll(@Param("name") Integer name,
													  @Param("createdFrom") Date createdFrom,
													  @Param("createdTo") Date createdTo,
													  @Param("taskCompletedFrom") Date taskCompletedFrom,
													  @Param("taskCompletedTo") Date taskCompletedTo,
...
Created On
	...
	@Override
	@RestResource(exported = false)
	void delete(AdminHourlyTaskEntity entity);
	...
	@Override
	@RestResource(exported = false)
	void deleteAll(Iterable<? extends AdminHourlyTaskEntity> entities);
