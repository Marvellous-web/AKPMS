...
public interface DoctorCompanyDataRestRepository extends PagingAndSortingRepository<DoctorCompanyEntity, Integer> {
...
//	@Query("SELECT i FROM AdminDoctorCompanyEntity i  ORDER BY i.name")
	@Cacheable
	public Page<DoctorCompanyEntity> findByNameContains(@Param("name") String name, Pageable pageable);
...
//	@Query("SELECT i FROM AdminDoctorCompanyEntity i where i.enabled = 1 ORDER BY i.name")
	@Cacheable
	public List<DoctorCompanyEntity> findByEnabledIsTrue();
...
//	@Query("SELECT i FROM AdminDoctorCompanyEntity i where i.enabled=:enabled  ORDER BY i.name")
	...
	Page<DoctorCompanyEntity> findByNameContainsAndEnabled(@Param("name") String name, @Param("enabled") Boolean enabled, Pageable pageable);
...
//	public Page<AdminDoctorCompanyEntity> findByQuery(@Param("query") String query, @Param("enabled") Boolean enabled, Pageable pageable);
	...
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "customquery", rel = "customquery")
	@Query("SELECT i FROM DoctorCompanyEntity i where  CONCAT(i.name) LIKE %:query% AND (:enabled is null or i.enabled = :enabled)   ORDER BY CASE WHEN :sortDirection = 'desc' " +
			" THEN CASE " +
			...
			" END ASC ")
	@Cacheable
	public Page<DoctorCompanyEntity> findByQuery(
	...
	@Query("SELECT i FROM DoctorCompanyEntity i where  CONCAT(i.name) LIKE %:query% AND (:enabled is null or i.enabled = :enabled) ORDER BY i.name")
	@Cacheable
	public List<DoctorCompanyEntity> findByQueryAll(@Param("query") String query, @Param("enabled") Boolean enabled);
	
	
	@Override
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@CachePut
	<S extends DoctorCompanyEntity> S save(S companyEntity);
	
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "exists", rel = "exists")
	@Query("SELECT i FROM DoctorCompanyEntity i where i.name=:name")
	@Cacheable(key = "#name")
	public DoctorCompanyEntity findByName(@Param("name") String name);

	@Query(value="SELECT count(i) FROM DoctorCompanyEntity i")
...
//	void delete(AdminDoctorCompanyEntity entity);
	...
	void delete(DoctorCompanyEntity entity);

	@Override
	@RestResource(exported = false)
	List<DoctorCompanyEntity> findAll();
	...
	@Override
	@RestResource(exported = false)
	void deleteAll(Iterable<? extends DoctorCompanyEntity> entities);
