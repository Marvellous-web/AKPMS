...
public interface DoctorGroupDataRestRepository extends PagingAndSortingRepository<AdminDoctorGroup, Integer> {
...
//	@Query("SELECT i FROM AdminDoctorGroup i  ORDER BY i.name")
	@Cacheable
	public Page<AdminDoctorGroup> findByNameContains(@Param("name") String name, Pageable pageable);
	

		//2.
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "allEnabled", rel = "allEnabled")
//	@Query("SELECT i FROM AdminDoctorGroup i where  i.enabled =:enabled ORDER BY i.name")
	@Cacheable(key = "#enabled-only")
	public Page<AdminDoctorGroup> findByNameContainsAndEnabled(@Param("name") String name, @Param("enabled") Boolean enabled, Pageable pageable);


//	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
//	@RestResource(path = "customquery", rel = "customquery")
//	@Query("SELECT i FROM AdminDoctorGroup i where  CONCAT(i.name) LIKE %:query% AND (:enabled is null or i.enabled = :enabled)  ORDER BY i.name")
//	@Cacheable
...
//
	...
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "customquery", rel = "customquery")
	@Query("SELECT i FROM AdminDoctorGroup i where  CONCAT(i.name) LIKE %:query% AND" +
	...
	@Cacheable
	public Page<AdminDoctorGroup> findByQuery(@Param("query") String query,
	...
	@Query("SELECT i FROM AdminDoctorGroup i where  CONCAT(i.name) LIKE %:query% AND (:enabled is null or i.enabled = :enabled)  ORDER BY i.name")
	@Cacheable
	public List<AdminDoctorGroup> findByQueryAll(@Param("query") String query, @Param("enabled") Boolean enabled, Pageable pageable);
	
	//@PreAuthorize("hasAnyAuthority('role_admin')")
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "allGroups", rel = "allGroups")
	@Cacheable(key = "#allGroups")
	public List<AdminDoctorGroup> findByEnabledIsTrue();

	@Override
	//@PreAuthorize("hasAuthority('role_admin')")
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@CachePut
	<S extends AdminDoctorGroup> S save(S DoctorGroup);
	
	//@PreAuthorize("hasAuthority('role_admin')")
	@PreAuthorize("hasAnyAuthority('role_admin','role_user')")
	@RestResource(path = "exists", rel = "exists")
	@Query("SELECT i FROM AdminDoctorGroup i where i.name=:name")
	@Cacheable(key = "#name")
	public AdminDoctorGroup findByName(@Param("name") String name);
	...
	@Query(value="select * from doctor_group i where i.doctor_company_id = :companyId",nativeQuery = true)
		public List<AdminDoctorGroup> findByCompany(@Param("companyId") Integer companyId);

	@Query(value="SELECT count(i) FROM AdminDoctorGroup i")
...
//	void delete(DoctorGroupEntity entity);
	...
	void delete(AdminDoctorGroup entity);
	...
	@Override
	@RestResource(exported = false)
	void deleteAll(Iterable<? extends AdminDoctorGroup> entities);
