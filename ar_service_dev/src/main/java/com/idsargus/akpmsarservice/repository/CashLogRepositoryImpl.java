...
@Repository
@Slf4j
public class CashLogRepositoryImpl implements CashLogRepository{
	...
	public List<Object[]> journalEntryManagementFee(String month, String year)
		...
		/*
		 * sb.append("SELECT " + "'7090' as revenueCode, " + "'' as revenueName, " +
		 * "doc.doctor_code as doctorCode, " + "doc.name as doctorName, " +
...
//		return em.createNativeQuery(sb.toString()).getResultList();
		...
		sb.append("SELECT "
				...
				+ "SUM(pmtBatch.deposit_amt+pmtBatch.ndba),"
				+ "ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.accounting + rev.operations + rev.payments) ),2) as mgmtfee, "
				+ "ROUND((SUM(pmtBatch.deposit_amt + pmtBatch.ndba)*0.01),2) as depositAmount,"
				+ "ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.payments) ),2) as mgmtfeePmt, "
				+ "ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.accounting ) ),2) as mgmtfeeAcc, "
				+ "ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.operations ) ),2) as mgmtfeeOpt ");
		sb.append(" FROM payment_batch pmtBatch ");
		...
		sb.append(" WHERE pmtBatch.company_id=58");
		sb.append(" AND pmtBatch.payment_type_id = 1");
//           + Constants.PAYMENT_TYPE_ADMIN_INCOME);
		sb.append(" AND pmtBatch.billing_month=" + month
				+ " AND pmtBatch.billing_year =" + year);
		sb.append(" GROUP BY doc.name WITH ROLLUP");
...
//				+ "doc.doctorCode as doctorCode, "
		...
		sb.append(" LEFT JOIN revenue_type rev ON pmtBatch.revenue_type_id = rev.id ");
		sb.append(" LEFT JOIN doctor doc ON pmtBatch.doctor_id = doc.id ");
		sb.append(" LEFT JOIN doctor_company docomp ON pmtBatch.company_id = docomp.id where pmtBatch.company_id = 58 ");
//		sb.append(" WHERE pmtBatch.doctor_id=" + Constants.PRO_HEALTH_GROUP_ID);
		sb.append(" AND pmtBatch.payment_type_id = "
				+ Constants.PAYMENT_TYPE_ADMIN_INCOME);
		sb.append(" AND pmtBatch.billing_month=" + month
...
//
//		SELECT '7090' as revenueCode, rev.name as revenueName, doc.doctor_code as doctorCode,
//		doc.name as doctorName, SUM(pmtBatch.deposit_amt+pmtBatch.ndba)
//		 ,ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.accounting + rev.operations + rev.payments) ),2) as mgmtfee,
//		 ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.payments) ),2) as mgmtfeePmt,
//		 ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.accounting ) ),2) as mgmtfeeAcc,
//		 ROUND(SUM((pmtBatch.deposit_amt+pmtBatch.ndba)*(rev.operations ) ),2) as mgmtfeeOpt
//		 FROM payment_batch pmtBatch
//		 LEFT JOIN revenue_type rev ON pmtBatch.revenue_type_id = rev.id
//		 LEFT JOIN doctor doc ON pmtBatch.doctor_id = doc.id
//		 LEFT JOIN doctor_company docomp ON pmtBatch.company_id = 58
//		  AND pmtBatch.payment_type_id = 1  AND pmtBatch.billing_month=10 AND pmtBatch.billing_year =2023 GROUP BY doc.name, rev.name;

		/* added here new code */
//
//		sb.append("SELECT "
//				//    + "rev.code as revenueCode, "
//				+ "'7090' as revenueCode,    pmtBatch.id as ticketNumber , "
//				+ "rev.name as revenueName, "
//				+ " doc.doctorCode as doctorCode, "
//				+ "doc.name as doctorName, "
...
//				+ " SUM(pmtBatch.deposit_amt + pmtBatch.ndba) as totalDeposit,"
...
//		sb.append(" LEFT JOIN revenue_type rev ON pmtBatch.revenue_type_id = rev.id ");
//		sb.append(" LEFT JOIN doctor doc ON pmtBatch.doctor_id = doc.id ");
//		sb.append(" WHERE pmtBatch.doctor_id=1");
//		sb.append(" AND pmtBatch.payment_type_id = 1");
////           + Constants.PAYMENT_TYPE_ADMIN_INCOME);
//		sb.append(" AND pmtBatch.billing_month=" + month
//				+ " AND pmtBatch.billing_year =" + year);
//		sb.append(" GROUP BY doc.name , pmtBatch.id  ORDER BY doc.name, ticketNumber ASC");
...
//		sb.append(" WHERE pmtBatch.doctor_id=" + Constants.PRO_HEALTH_GROUP_ID);
		...
		sb.append(" AND pmtBatch.billing_month=" + month
				+ " AND pmtBatch.billing_year =" + year);
		sb.append(" GROUP BY doc.name,  pmtBatch.id  WITH ROLLUP ");
	...
	public List<Object[]> dailyPaymentReceiptLog(String month, String year,
		...
		// sb.append(" UNION ");
		// sb.append(" (SELECT pmtBatch.id, rev.code as revenueCode, mnySrc.name as moneySourceName, rev.name as revenueName, doc.doctor_code as doctorCode, "
		// +
...
//				+ Constants.PAYMENT_TYPE_ADMIN_INCOME);
		...
		// date deposit from
		if (depositDateFrom != "") {
			log.info("found DATE_DEPOSIT_FROM = " + depositDateFrom);
		...
		// date deposit To
		if (depositDateTo != "") {
			log.info("found DATE_DEPOSIT_TO = " + depositDateTo);
			sb.append(" AND DATE_FORMAT(pmtBatch.deposit_date, '%Y-%m-%d') <= '"
					+ AkpmsUtil.akpmsDateFormat(
	...
	//	sb.append(" GROUP BY pmtBatch.id ");
		sb.append("  GROUP BY  doctorName, depositDate, revenueType, money_source_id");
		sb.append(" UNION ALL ");
