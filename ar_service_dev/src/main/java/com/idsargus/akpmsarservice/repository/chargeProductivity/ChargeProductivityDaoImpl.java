...
public class ChargeProductivityDaoImpl implements ChargeProductivityDao {
	...
	private static final Logger LOGGER = Logger
			.getLogger(String.valueOf(ChargeProductivityDaoImpl.class));
	...
	public int totalRecord(Map<String, String> whereClauses)
		...
		String datePostedTo = whereClauses.get("postingDateTo");
		String datePostedFrom = whereClauses.get("postingDateFrom");
		List<Object> ret = new ArrayList<>();
		StringBuilder queryString = new StringBuilder();
		queryString
				.append("SELECT count(*) FROM charge_productivity AS chgProd join charge_batch_processing cb on chgProd.ticket_number = cb.id " +
						"WHERE 1=1  AND MONTH(chgProd.scan_date) = "+month+"  AND YEAR(chgProd.scan_date) = "+year+" AND " +
						"chgProd.productivity_type = 'CE'" +
						" AND cb.date_posted >= '"+datePostedFrom+"' and cb.date_posted <= '"+datePostedTo+"'");
	...
	public List<ChargeProductivity> findAll(Map<String, String> orderClauses,
		...
		String datePostedTo = whereClauses.get("postingDateTo");
		String datePostedFrom = whereClauses.get("postingDateFrom");
		List<ChargeProductivity> ret = new ArrayList<>();
		StringBuilder queryString = new StringBuilder();

		queryString
				.append("SELECT chgProd.id,  chgProd.ticket_number, chgProd.productivity_type, " +
						"chgProd.remarks,chgProd.work_flow," +
						"chgProd.scan_date FROM charge_productivity AS chgProd join charge_batch_processing cb on chgProd.ticket_number = cb.id " +
						"WHERE 1=1  AND MONTH(chgProd.scan_date) = "+month+"  AND YEAR(chgProd.scan_date) = "+year+" AND " +
						"chgProd.productivity_type = 'CE'" +
						" AND cb.date_posted >= '"+datePostedFrom+"' and cb.date_posted <= '"+datePostedTo+"'");
