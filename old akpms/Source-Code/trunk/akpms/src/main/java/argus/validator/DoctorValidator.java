...
public class DoctorValidator extends LocalValidatorFactoryBean implements
	...
	private static final Logger LOGGER = Logger.getLogger(DoctorValidator.class);
	...
	// Pattern doctorNamePattern) {
		...
		/*
		 * Matcher doctorCode =
		 * doctorNamePattern.matcher(doctor.getDoctorCode()); if
		 ...
		 * (!doctorCode.matches())) { errors.rejectValue(Constants.DOCTOR_CODE,
		 * "doctor.characters"); }
		 */
	...
	public void validate(Object target, Errors errors) {
		...
		try {
			...
			if (null != doctor.getParent().getId()) {
				isDoctor = doctorDao.findByName(doctor.getName(), doctor
						.getParent().getId());
			} else {
				isDoctor = doctorDao.findByName(doctor.getName(), null);
			}
		...
		/*
		 * try { Doctor isDoctor =
		 * doctorDao.findByDoctorCode(doctor.getDoctorCode() .trim(), false); if
		 * (isDoctor != null) { if (doctor.getId() == null) {
		 * LOGGER.info("duplicate doctor code validation in case of add doctor"
		 * ); errors.rejectValue(Constants.DOCTOR_CODE, "doctor.code.exist"); }
		 * else if (doctor.getId().longValue() != isDoctor.getId() .longValue())
		 * {
		 * LOGGER.info("duplicate doctor code validation in case of edit doctor"
		 * ); errors.rejectValue(Constants.DOCTOR_CODE, "doctor.code.exist"); }
		 * }
		 *
		 * } catch (Exception e) { LOGGER.info( "No Doctor Exists with code = "
		 * + doctor.getDoctorCode(), e); }
		 */
