...
public class PaymentTypeDaoImpl implements PaymentTypeDao {
	...
	private static final Logger LOGGER = Logger
			.getLogger(PaymentTypeDaoImpl.class);
	...
	private static final String PAYMENT_TYPE_STATS_NATIVE = "SELECT status, COUNT(*) FROM payment_type i WHERE i.deleted = 0 GROUP BY status";

	@Autowired
	private EntityManager em;

	public PaymentType findById(Long id) throws ArgusException {
		return em.find(PaymentType.class, id);
	}

	public PaymentType findByName(String name) throws ArgusException {
	...
	public void addPaymentType(PaymentType paymentType) throws ArgusException {
	...
	@Override
	public int deletePaymentType(List<Long> ids) throws ArgusException {
	...
	/*
	 * @Override public PaymentType findByNameAndType(String name, String type)
	 * {
	 * 
	 * CriteriaBuilder builder = em.getCriteriaBuilder();
	 * CriteriaQuery<PaymentType> criteria = builder
	 * .createQuery(PaymentType.class); Root<PaymentType> paymentType =
	 * criteria.from(PaymentType.class);
	 * 
	 * criteria.select(paymentType).where(
	 * builder.equal(paymentType.get("name"), name),
	 * builder.equal(paymentType.get("type"), type)); return
