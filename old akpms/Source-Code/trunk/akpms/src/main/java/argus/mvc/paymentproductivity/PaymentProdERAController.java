...
public class PaymentProdERAController {
	...
	private static final Logger LOGGER = Logger
			.getLogger(PaymentProdERAController.class);
		...
		if (!result.hasErrors()) {
			if (payProductivity.getWorkFlowId() == Constants.QUERY_TO_TL_WORK_FLOW
				...
				map.put(Constants.TICKET_NUMBER, payProductivity
						.getPaymentBatch().getId());
				if (null != payProductivity.getPaymentBatch().getInsurance()) {
					map.put(Constants.INSURANCE_NAME, payProductivity
							.getPaymentBatch().getInsurance().getName());
				} else {
					map.put(Constants.INSURANCE_NAME, "");
				}
			}
			...
			if (payProductivity.getId() != null) {
				try {
					...
					paymentBatch.setComment(payProductivity
							.getPaymentBatch().getComment());
				...
				try {
					...
					paymentBatch.setDatePosted(payProductivity
							.getPaymentBatch().getDatePosted());
					paymentBatch.setComment(payProductivity.getPaymentBatch().getComment());
					batchDao.updatePaymentBatch(paymentBatch);
					ticketNumber[0] = paymentBatch.getId();
					session.setAttribute(
							Constants.SUCCESS_UPDATE,
	...
	private List<PaymentProductivityJsonData> getJsonData(
		...
		if (rows != null && rows.size() > 0) {
			...
			for (PaymentProductivity paymentProductivity : rows) {
				...
				paymentProductivity.setCreatedOn(TimeConverter.convertTime(paymentProductivity.getCreatedOn()));
				paymentProductivity.setModifiedOn(TimeConverter.convertTime(paymentProductivity.getModifiedOn()));
				...
				if(paymentProductivity.getPaymentBatch()
						.getDoctor().isNonDeposit()){
				djd.setDoctorName(paymentProductivity.getPaymentBatch()
						.getDoctor().getName()+" (Non-Deposit)");
				}
				else{
					djd.setDoctorName(paymentProductivity.getPaymentBatch()
							.getDoctor().getName());
				...
				djd.setPostedBy(paymentProductivity.getCreatedBy()
						.getFirstName()
						+ " "
