...
<div class="dashboard-container" >
	...
	<form class="form-container" id="qcPointList">
		<div class="form-container-inner">
			<h3>Search QC points</h3>
...
<script type="text/javascript">
	var moduleName = "qcpoint";
		...
		function addFormData() {
			...
			var dt = $('#qcPointList').serializeArray();
			$("#flex1").flexOptions({
				params : dt
			});
			return true;
		...
		function formatData(data) {
			...
			$.each(data.rows, function(i, row) {
				...
				if(row.status == false && row.parentName != "--" && row.parentStatus == "false"){
					msg = 'Please activate parent QC Point before editing.';
					edit = "<a href='javascript:void(0)' onclick='javascript:alert(\"Please activate parent QC Point before editing.\");'><img src='"+editIcon+"' /></a>";
				}else{
				...
				rows.push({
					id : row.id,
					cell : [edit, row.name, row.parentName, row.department]
				});
			});
		...
		function checkStatus(status, parentStatus){
			...
			if(status == 0 && parentStatus == 0){
				alert('Please activate parent QC Point before editing.');
				return false;
			}
		}
		...
		function send_delete(id, childCount){
			...
			if(childCount > 0 ){
				...
				confirmation = confirm("Deleting this QC point will also delete its "+childCount+" sub-QC point. Are you sure you wish to proceed?");
			}else{
				confirmation = confirm("Are you sure you wish to delete this QC point?");
			}
			...
			if (confirmation) {
				$.ajax({
					...
					success : function(data) {
						$("#flex1").flexReload();
						$("#msgP").text("QC point(s) have been successfully deleted.");
					}
				});
			}
		}
		...
		function send_changeStatus(id, status, childCount, parentStatus) {
			...
			if(childCount > 0 && status == 0){
				...
				confirmation = confirm("Deactivating this QC Point will also deactivate its "+childCount+ " sub-departments. Are you sure you wish to proceed?");
				...
				alert ("Please activate its parent QC Point before activating this department.");
				return true;
			}else{
				confirmation = confirm("Are you sure you wish to "+st+" this QC Point?");
			}
			...
			if(confirmation){
				$.ajax({
					...
					success : function(data) {
						...
						$("#flex1").flexReload();
							$("#msgP").text("QC Point(s) have been successfully "+st+"d.");
					},
					error: function(data){
						alert (data);
					}
