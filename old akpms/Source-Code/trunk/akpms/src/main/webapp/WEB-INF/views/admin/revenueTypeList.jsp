...
<div class="dashboard-container">
	...
	<form class="form-container" id="revenueTypeList">
		<div class="form-container-inner">
			<h3>Search Revenue Type</h3>
...
<script type="text/javascript">
	...
	function addFormData() {
		...
		var dt = $('#revenueTypeList').serializeArray();
		$("#flex1").flexOptions({
			params : dt
		});
		return true;
	...
	$('#revenueTypeList').submit(function() {
		$('#flex1').flexOptions({
			newp : 1
		}).flexReload();
		return false;
...
//				del = "<a href='javascript:void(0)' onClick='send_delete(" + row.id + ")' class='delete'><img src='"+deleteIcon+"' alt='Delete' title='Delete' /></a>";
				...
				rows.push({
					id : row.id,
					cell : [edit, row.name, row.code, row.desc]
				});
			});
	...
	/* function send_delete(id){
		if (confirm("Are you sure you wish to delete the selected revenue type?")) {
			$.ajax({
				type : "POST",
				dataType : "json",
				...
				success : function(data) {
					$("#flex1").flexReload();
					$("#msgP").text("Revenue type has been successfully deleted.");
				}
			});
		}
	} */
	...
	function send_changeStatus(id, status) {
		...
		if(confirm("Are you sure you wish to "+st+" this revenue type?")){
			$.ajax({
				type : "GET",
				dataType : "json",
				...
				success : function(data) {
					...
					$("#flex1").flexReload();
					$("#msgP").text("Revenue type has been successfully "+st+"d.");
				},
				error : function(data) {
					alert(data);
				}
