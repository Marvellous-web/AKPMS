...
<div class="dashboard-container">
	...
	<form class="form-container" id="locationList">
...
<script type="text/javascript">
	...
	function addFormData() {
		...
		var dt = $('#locationList').serializeArray();
		$("#flex1").flexOptions({
			params : dt
		});
		return true;
...
//			del = "<a href='javascript:void(0)' onClick='send_delete(" + row.id + ")' class='delete'><img src='"+deleteIcon+"' alt='Delete' title='Delete' /></a>";
	...
	function send_delete(id){
		if (confirm("Are you sure you wish to delete the selected location?")) {
			$.ajax({
				type : "POST",
				dataType : "json",
				...
				success : function(data) {
					$("#flex1").flexReload();
					$("#msgP").text("Location has been successfully deleted.");
				}
			});
		}
	}
	...
	function send_changeStatus(id, status) {
		...
		if(confirm("Are you sure you wish to "+st+" this location?")){
			$.ajax({
				type : "GET",
				dataType : "json",
				...
				success : function(data) {
					...
					$("#flex1").flexReload();
					$("#msgP").text("Location has been successfully "+st+"d.");
				},
				error : function(data) {
					alert(data);
				}
