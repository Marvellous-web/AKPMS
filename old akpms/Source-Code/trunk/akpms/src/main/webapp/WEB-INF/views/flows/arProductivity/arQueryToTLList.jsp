	...
	<form class="form-container" id="sform">
		<div class="form-container-inner">
			...
			<span class="input-container"> <label>&nbsp;</label> <input
				...
				<input type="reset" title="Cancel" value="Cancel"
				class="login-btn" />
			</span>
		...
		$.each(data.rows,function(i, row) {
			...
			rows.push({
				...
				cell : [ row.patientAccNo, row.patientName, row.dos, row.cpt, row.doctor,
						 row.insurance, row.dataBas, row.balanceAmt, row.remark,
						 row.tlRemark, row.sourceName, row.workFlowName,
						 edit]
			});
		});
