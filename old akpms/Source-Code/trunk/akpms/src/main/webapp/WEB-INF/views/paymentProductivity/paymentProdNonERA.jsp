...
<div class="dashboard-container">
	...
	<div class="section-content">
	    ...
	    <div class="row-container">
	    	<div class="element-block" >
		    	<span class="lbl">Payment Type:</span> <span id="paymentType" ></span>
		    </div>
	    	<div class="clr"></div>
	    </div>
	</div>
	...
	<div class="form-container" id="divProductivityForm">
		<div class="form-container-inner">
			<h3>${operationType} ${prodType}</h3>
			<div class="row-container">
				<div class="element-block" >
					<form:label path="paymentBatch.manuallyPostedAmt">Posted in CT:<em class="mandatory">*</em><br/>&nbsp;&nbsp;&nbsp;(in USD)</form:label>
					<div class="input_text">
						<span class="left_curve"></span>
			...
			<div class="row-container">
				<div class="element-block" >
					<form:label path="manualTransaction">Total Transactions (Manual):<em class="mandatory">*</em></form:label>
					<div class="input_text">
						<span class="left_curve"></span>
			...
			<div class="row-container">
				<div class="element-block" >
					...
					<div class="input_text">
						<span class="left_curve"></span>
							<form:input path="paymentBatch.datePosted" id="datePosted" cssClass="mid" cssStyle="width: 75px;" readonly="true" />
						<span class="right_curve"></span>
					</div>
				...
				<div class="element-block" >
					<form:label path="paymentBatch.suspenseAccount">Suspense:<br/>&nbsp;&nbsp;&nbsp;(in USD)</form:label>
					<div class="input_text">
						<span class="left_curve"></span>
			...
			<div class="row-container">
				<div class="element-block" >
					<form:label path="time">Time (min):</form:label>
					<div class="input_text">
						<span class="left_curve"></span>
			...
			<div class="row-container">
				...
				<form:textarea rows="5" cols="20" path="paymentBatch.comment" id="remark"/>
				<div class="clr"></div>
			</div>
            ...
            <div class="row-container">
				<form:label path="workFlowId">Work Flow:</form:label>
				<div>
					<form:select path="workFlowId" cssClass="selectbox" onchange="javascript:checkOffsetWithWorkflow();">
						...
						<form:option value="7">To AR MCR</form:option>
						<form:option value="8">To AR WC</form:option>
			...
			<div class="row-container">
			 <label>&nbsp;</label>
				...
				value="${buttonName}" class="login-btn" onclick="javascript:disabledButtons();this.form.submit();"/> <input type="button"
				title="Cancel" value="Cancel" class="login-btn"  onclick="javascript:resetForm(this.form);setTicketNumber();totalCTPosted();" /> <c:if
...
<c:if test="${paymentProductivity.workFlowId == 6}">
	<script type="text/javascript">
		isQueryTOTL = true;
	</script>
</c:if>
			...
			success : function(data) {
				if(data != null && data.length > 0){
					if(data[0] == "err"){
						...
						if(data[0].parentDoctor != ""){
							doctor += " ("+data[0].parentDoctor+")";
						}
						$('#doctor').text(doctor);
						$("#paymentType").text(data[0].paymentType);
...
function checkBatchDetails(){
	if($('#paymentBatchId').val() > 0){
		...
		if(paymentProdId == null || $.trim(paymentProdId) == ''){
			...
			var doctor = "${paymentProductivity.paymentBatch.doctor.name}";
			if("${paymentProductivity.paymentBatch.doctor.nonDeposit}") {
				doctor += " (Non-Deposit)";
			}
			if("${paymentProductivity.paymentBatch.phDoctor}" != ""){
				doctor += " (${paymentProductivity.paymentBatch.phDoctor.name})";
			}
			$('#doctor').text(doctor);
			
			$('#paymentType').text("${paymentProductivity.paymentBatch.paymentType.name}");
			$('#insurance').text("${paymentProductivity.paymentBatch.insurance.name}");
			$('#dateRecieved').text('${currentDate}');
			var amount = "${paymentProductivity.paymentBatch.depositAmount}";
			if(amount == 0){
				$('#depositAmount').text(parseFloat("${paymentProductivity.paymentBatch.ndba}").toFixed(2));
			}else{
				$('#depositAmount').text(parseFloat("${paymentProductivity.paymentBatch.depositAmount}").toFixed(2));
			}
		}
	}
}
...
function totalCTPosted(){
	var manuallyPostedAmt = $('#manuallyPostedAmt').val();
    if(manuallyPostedAmt == "" || manuallyPostedAmt == "."){
    	manuallyPostedAmt = 0.0;
	}
	var totalCTPostedAmt = parseFloat(manuallyPostedAmt);
