...
<div class="dashboard-container">
	...
	<div id="bread-crumb">
		<ul>
			...
			<li>&nbsp;${operationMode} QA Worksheet Staff</li>
		</ul>
	</div>
	<div class="clr"></div>
	<!-- Bread Crumb End-->
	...
	<%-- qaworksheetAddStaff.jsp  --%>
	<form:form method="post" modelAttribute="qaworksheetStaff" class="form-container"  id="form1">
		<div class="form-container-inner">
			<h3 class="heading-h3">Add Staff to QA Worksheet</h3>
			<div class="row-container">
				<div class="element-block">
					<form:label path="user.id">Select Staff:<em class="mandatory">*</em></form:label>
					<span class="select-container2" style="display:inline;">
						<form:select path="user.id" class="selectbox">
					...
					<form:errors path="user.id" cssClass="invalid" />
				</div>
				
				<form:hidden path="qaWorksheet.id" value="${qaworksheet.id}"/>
				<div class="element-block" id="divGeneralBox" style="visibility:inline;">
					<form:label path="percentageValue">Staff Percentage:<em class="mandatory">*</em></form:label>
					<span class="input_text">
						<span class="left_curve"></span>
							<form:input path="percentageValue" type="text" name="staffPercentage" id="percentage" cssClass="mid numbersOnly"
			...
			<div class="row-container">	
				...
				<div class="element-block" >
					<input type="submit" id="addToList" class="login-btn" value="Add Staff"/>
				</div>
				<div class="clr"></div>
			</div>
		</div>
...
<script type="text/javascript">
	 $("#flex1").flexigrid({
			url : contextPath + '/qamanager/staffjson',
		...
		function formatData(data) {
			...
			$.each(data.rows, function(i, row) {
				...
				rows.push({
					cell : [del, row.recordPercentage + '%', row.userName, row.remarks]
				});
			});
		...
		function send_delete(id){
			$.ajax({
				type : "POST",
				dataType : "json",
				url : contextPath + "/qamanager/deletestaff",
				data : "item=" + id,
				success : function(data) {
					if (data) {
						$("#flex1").flexReload();
						$("#msgP").text("Staff have been successfully deleted.");
					} else {
						$("#msgP").text("Failed to delete Staff.");
					}
				}
			});
		};
