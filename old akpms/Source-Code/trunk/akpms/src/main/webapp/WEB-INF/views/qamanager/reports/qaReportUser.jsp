...
<div class="dashboard-container">
	...
	<div class="table-grid" style="overflow-x: auto;">
		<table
			style="font-size: 9px !important; table-layout: fixed; margin: auto">
			<c:set var="totalQARecords" value="${0}" />
			...
			<thead>
				<tr>
					...
					<c:choose>
						<c:when test="${not empty reportData}">
								...
								<c:choose>
									<c:when test="${empty dataRow[1] && not empty dataRow[2]}">
										<tr>
											...
											<td style="text-align: center" title="QARecord"><fmt:parseNumber
													integerOnly="true" type="number" value="${dataRow[5]}" />
												<c:set var="totalQARecords"
													value="${totalQARecords + dataRow[5]}" /></td>
											<td style="text-align: center" title="totalRecord">${dataRow[6]}
												<c:set var="totalRecords" value="${totalRecords+dataRow[6]}" />
											</td>

											<td style="text-align: center" title="totalErrors">${dataRow[7]}
												<c:set var="totalErrors" value="${totalErrors + dataRow[7]}" />
											</td>
											<td style="text-align: center" title="errorsRatePercentage">${dataRow[8]}%</td>
										</tr>
										<tr>
											<td colspan="6">&nbsp;</td>
										</tr>
									...
									<c:when test="${not empty dataRow[2]}">
										<tr>
											...
											<!--  <td style="text-align: center" title="totalRecord">${dataRow[4]}</td> -->
											<td style="text-align: center" title="QARecord"><fmt:parseNumber
													integerOnly="true" type="number" value="${dataRow[5]}" /></td>
											<td style="text-align: center" title="totalRecord">${dataRow[6]}</td>
							...
							<tr>
								...
								<td style="text-align: center; font-weight: bold"
									title="QARecord"><c:out value="${totalQARecords}" />
									</td>
								<td style="text-align: center; font-weight: bold"
								...
								<c:set var="totalPercent"
									value="${(totalErrors*100)/totalQARecords}" />
						...
						<c:otherwise>
							<tr align="center">
								<td style="color: red; font-weight: bold" colspan="6">NO
									RECORD FOUND</td>
							</tr>
						</c:otherwise>
					</c:choose>
