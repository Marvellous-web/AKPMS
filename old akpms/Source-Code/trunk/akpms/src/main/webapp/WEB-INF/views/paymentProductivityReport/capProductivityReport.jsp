...
<script type="text/javascript">
	...
	$("#flex1").flexigrid({
		url : contextPath + '/'+moduleName+'/capReportJson',
	...
	function formatData(data) {
		...
		$.each(data.rows,function(i, row) {
            rows.push({
					id : row.id,
					cell : [ row.paymentBatch, row.manuallyPostedAmt,row.depositAmt,row.suspense,row.doctorName,row.insurance,row.datePosted, row.receivedDate,row.time,row.postedBy ]
				});
			});
