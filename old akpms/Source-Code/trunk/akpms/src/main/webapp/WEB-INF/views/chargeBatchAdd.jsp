...
<div class="dashboard-container">
	...
	<!-- Bread Crumb End-->
			...
			<div class="section-content">
				...
				<div class="row-container">
					<div class="element-block" >
				    	<span  class="lbl">Batch/Ticket No.:</span> <span style="font-size: 20px;">${chargeBatchProcess.id}</span>
				    </div>
			    	<div class="clr"></div>
				</div>
				...
				<div class="row-container">
				    ...
				    <div class="element-block" >
				    	...
				    	<span>
				    		${chargeBatchProcess.doctor.name}
				    		<c:if test="${not empty chargeBatchProcess.doctor.parent}">(${chargeBatchProcess.doctor.parent.name})</c:if>									    	
				    	</span>
				    </div>
			    	<div class="clr"></div>
				</div>
		...
		<c:if test="${not empty chargeBatchProcess.dateReceived}">
			<div class="section-content">
			    ...
			    <div class="row-container">
				    <div class="element-block" >
				    	<span class="lbl">Received By:</span> <span>${chargeBatchProcess.receivedBy.firstName} ${chargeBatchProcess.receivedBy.lastName}</span>
				    </div>
				    <div class="element-block" >
		...
		<form:form commandName="chargeBatchProcess" id="form1" >
			<div class="form-container"  <c:if test="${operationMode eq 'View'}">style="display:none;"</c:if> >
				<c:if test="${operationType == 'Add'}">
					...
					<h3>Doctor Office</h3>
					<div class="row-container">
						<form:label path="doctor.id">Doctor/Group Name:<em class="mandatory">*</em></form:label>
						<span class="select-container" style="display:inline;">
							...
							<!--	<c:forEach var="doctor" items="${doctorList}">
								...
								<c:forEach var="doctor" items="${doctorList}">
									<form:option value="${doctor.id}">${doctor.name}
					...
					<div class="row-container">
						<div class="element-block">
							...
							<span class="input_text">
								<span class="left_curve"></span>
									<form:input path="dosFrom" readonly="true" cssClass="mid" cssStyle="width: 90px;" />
						...
						<div class="element-block">
							...
							<span class="input_text">
								<span class="left_curve"></span>
									<form:input path="dosTo" readonly="true" cssClass="mid" cssStyle="width: 90px;"  />
								<span class="right_curve"></span>
							</span>
					...
					<div class="row-container">
						<div class="element-block">
							...
							<span class="input_text">
								<span class="left_curve"></span>
								<form:input path="numberOfSuperbills" maxlength="8" cssClass="mid integerOnly" cssStyle="width: 90px;"  onblur="javascript:calculatePages();" onkeypress="javascript:calculatePages();"  />
								<span class="right_curve"></span>
							</span>
						...
						<div class="element-block">
							...
							<span class="input_text">
								<span class="left_curve"></span>
								<form:input path="numberOfAttachments" maxlength="8" cssClass="mid integerOnly" cssStyle="width: 90px;" onblur="javascript:calculatePages();" onkeypress="javascript:calculatePages();" />
								<span class="right_curve"></span>
							</span>
					...
					<div class="row-container">
					    <div class="element-block" >
					    	<label class="lbl">Total no. of Pages:</label> <span id="spnTotalPages">${chargeBatchProcess.numberOfSuperbills + chargeBatchProcess.numberOfAttachments}</span>
					    </div>
				    	<div class="clr"></div>
				    </div>
			...
			<c:if test="${operationType == 'Edit' && empty chargeBatchProcess.postedBy}">
				...
				<div class="form-container-inner">
					...
					<div class="row-container">
						...
						<span class="input_text">
							<span class="left_curve"></span>
							<form:input path="dateReceived" readonly="true" cssClass="mid" cssStyle="width: 90px;"  />
							<span class="right_curve"></span>
						</span>
					...
					<div class="row-container">
						<form:label path="receivedBy.id">Received By:<em class="mandatory">*</em></form:label>
						<span class="select-container" style="display:inline;">
							<form:select path="receivedBy.id" cssClass="selectbox" id="receivedById">
								<form:option value="">-: Select Received By :-</form:option>
								<c:forEach items="${receivedByUsers}" var="receivedBy">
									<form:option value="${receivedBy.id}">${receivedBy.firstName} ${receivedBy.lastName}</form:option>
								</c:forEach>
							</form:select>
						</span>
						<form:errors class="invalid" path="receivedBy" />
						<div class="clr"></div>
					</div>
					...
					<div class="row-container">
						<div class="element-block">
							...
							<span class="input_text">
								<span class="left_curve"></span>
								<form:input path="numberOfSuperbillsArgus" maxlength="8" cssClass="mid integerOnly" cssStyle="width: 90px;"    onblur="javascript:calculatePagesArgus();" onkeypress="javascript:calculatePagesArgus();"  />
								<span class="right_curve"></span>
							</span>
						...
						<div class="element-block">
							...
							<span class="input_text">
								<span class="left_curve"></span>
								<form:input path="numberOfAttachmentsArgus" maxlength="8" cssClass="mid integerOnly" cssStyle="width: 90px;"   onblur="javascript:calculatePagesArgus();" onkeypress="javascript:calculatePagesArgus();"  />
								<span class="right_curve"></span>
							</span>
					...
					<div class="row-container">
					    <div class="element-block" >
					    	<label class="lbl">Total no. of Pages:</label> <span id="spnTotalPagesArgus">${chargeBatchProcess.numberOfSuperbillsArgus + chargeBatchProcess.numberOfAttachmentsArgus}</span>
					    </div>
				    	<div class="clr"></div>
				    </div>
			...
			<sec:authorize ifAnyGranted="P-10">
				<c:if test="${(operationType == 'Edit') && (not empty chargeBatchProcess.postedBy)}">
					...
					<input type="hidden" name="receivedById" value="${chargeBatchProcess.receivedBy.id}" />
					<div class="form-container-inner">
						<div class="row-container">
							<div class="element-block">
								...
								<span class="select-container2" style="display:inline;">
									<form:select path="postedBy.id" cssClass="selectbox" id="reupdatePostedBy">
										<c:forEach items="${postedByUsers}" var="postedBy">
											<form:option value="${postedBy.id}">${postedBy.firstName} ${postedBy.lastName}</form:option>
										</c:forEach>
									</form:select>
								</span>
							...
							<div class="element-block">
								...
								<span class="input_text">
									<span class="left_curve"></span>
										<form:input path="dateBatchPosted" readonly="true" cssClass="mid" cssStyle="width: 90px;" />
									<span class="right_curve"></span>
								</span>
					...
					<script type="text/javascript" >
						$('#reupdatePostedBy').val(${chargeBatchProcess.postedBy.id});
					</script>
				</c:if>
			</sec:authorize>
		</div>
...
function calculatePages(){
	var sb = $("#numberOfSuperbills").val();
	if($.trim(sb) == "")sb = 0;
	var at = $("#numberOfAttachments").val();
	if($.trim(at) == "")at = 0;
	$("#spnTotalPages").text(parseInt(sb) + parseInt(at));
}
function calculatePagesArgus(){
	var sb = $("#numberOfSuperbillsArgus").val();
	if($.trim(sb) == "")sb = 0;
	var at = $("#numberOfAttachmentsArgus").val();
	if($.trim(at) == "")at = 0;
	$("#spnTotalPagesArgus").text(parseInt(sb) + parseInt(at));
}
$(function() {
    $( "#dosFrom").datepicker();
    $( "#dosTo").datepicker();
