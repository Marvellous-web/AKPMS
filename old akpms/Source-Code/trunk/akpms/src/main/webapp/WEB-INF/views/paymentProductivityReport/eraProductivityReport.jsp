...
var contextPath = "<c:out value="${pageContext.request.contextPath}" />";
	var moduleName = "paymentproductivityERA";
	$("#flex1").flexigrid({
		...
		colModel : [ {
			...
			display : 'Manually Posted Amount',
	...
	function formatData(data) {
		...
		$.each(data.rows,function(i, row) {
            rows.push({
					id : row.id,
					cell : [ row.paymentBatch, row.manuallyPostedAmt,row.elecPostedAmt,row.depositAmt,row.doctorName,row.insurance,row.datePosted, row.receivedDate,row.time,row.postedBy,row.remark ]
				});
			});
