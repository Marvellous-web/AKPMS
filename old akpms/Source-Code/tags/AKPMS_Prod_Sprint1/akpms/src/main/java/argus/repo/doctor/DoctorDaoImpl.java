...
public class DoctorDaoImpl implements DoctorDao {
	private static final Log LOGGER = LogFactory.getLog(DoctorDaoImpl.class);
	...
	private static final String DOCTOR_STATS_NATIVE = "SELECT status, COUNT(*) FROM akpms.doctor d WHERE d.deleted = "
	...
	@Override
	public Doctor findByDoctorCode(String doctorCode, boolean dependancies)
			throws ArgusException {
		CriteriaBuilder builder = em.getCriteriaBuilder();
		CriteriaQuery<Doctor> criteria = builder.createQuery(Doctor.class);
		...
		criteria.select(doctor).where(
				builder.equal(doctor.get("doctorCode"), doctorCode),
				builder.equal(doctor.get("deleted"), false));
