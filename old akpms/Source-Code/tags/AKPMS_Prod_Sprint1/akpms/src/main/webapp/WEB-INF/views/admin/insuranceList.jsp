...
<div class="dashboard-container">
	...
	<form class="form-container" id="insuranceList">
...
<script type="text/javascript">
	...
	function addFormData() {
		...
		var dt = $('#insuranceList').serializeArray();
		$("#flex1").flexOptions({
			params : dt
		});
		return true;
	...
	function formatData(data) {
		...
		$.each(data.rows,function(i, row) {
			...
			rows.push({
				id : row.id,
				cell : [ row.name, row.desc, status, edit, del ]
			});
		});
	...
	function send_delete(id){
		if (confirm("Are you sure you wish to delete the selected insurance?")) {
			$.ajax({
				type : "POST",
				dataType : "json",
				...
				success : function(data) {
					$("#flex1").flexReload();
					$("#msgP").text("Insurance has been successfully deleted.");
				}
			});
		}
	}
	...
	function send_changeStatus(id, status) {
		...
		if(confirm("Are you sure you wish to "+st+" this insurance?")){
			$.ajax({
				type : "GET",
				dataType : "json",
				...
				success : function(data) {
					...
					$("#flex1").flexReload();
					$("#msgP").text("Insurance has been successfully "+st+"d.");
				},
				error : function(data) {
					alert(data);
				}
