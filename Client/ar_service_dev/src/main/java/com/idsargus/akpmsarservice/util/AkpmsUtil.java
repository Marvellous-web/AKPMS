...
public class AkpmsUtil {
	...
	public static Date akpmsNewDateFormat(String date) {
		...
		log.info("in akpmsDateFormat " + date);

		try {
			Date d = (Date) formatter.parse(date);

			log.info("date formatted " + d.toString());
			return d;
		} catch (Exception e) {
	...
	public static String akpmsDateFormat(Date date, String format) {
		...
		try {
			...
			log.info("date formatted " + dateAsString);
...
//	}
	...
	public static boolean checkPermission(String permissionId) {
		Set<PermissionEntity> permissions = getLoggedInUser().getPermissions();
	...
	public static Map<String, String> getPropertyMap(String fileName)
			throws Exception {
		log.debug("in getPropertyMap " + fileName);
		...
		log.debug("out getPropertyMap " + propMap.size());
		return propMap;
	}
	 ...
	 * log.debug("in commanSepString " + inputStr); HashSet<Long>
	 ...
	 *
	 * log.debug("out commanSepString " + outputString); return outputString;
	 * }
	 */
	...
	public static Map<String, String> getOrderClause(WebRequest request,
		...
		if (request.getParameter(Constants.RECORD_PRE_PAGE) != null) {
			...
			try {
				...
				log.debug("rp[Record pre Page] not coming or not an integer in request ");
			}
		}
		...
		if (request.getParameter(Constants.PAGE) != null) {
			try {
				...
				log.debug("Exception during parsing");
			}
		}
