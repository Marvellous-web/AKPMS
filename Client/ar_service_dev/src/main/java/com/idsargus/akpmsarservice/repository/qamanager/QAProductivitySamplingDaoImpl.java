...
import com.idsargus.akpmsarservice.repository.QAProductivitySamplingRepository;
...
//			.getLogger(QAProductivitySamplingDaoImpl.class);
	...
	@Autowired
	QAProductivitySamplingRepository qaProductivitySamplingRepository;
	...
	public List<QAProductivitySampling> findAll(
		...
		if (whereClause != null && whereClause.containsKey("search_sample")
			...
			queryString
					.append(" LEFT JOIN qaWorksheetPatientInfos patientInfo ");

			queryString
					.append(" LEFT JOIN qaWorksheet qaworksheet ");
	...
	public void save(QAProductivitySampling qaProductivitySampling)
		...
		qaProductivitySamplingRepository.save(qaProductivitySampling);
	}
...
//					+ queryString);
	...
	public List<Object[]> findQAWorksheetUserReportData(
		...
		if (whereClause != null && whereClause.size() > Constants.ZERO) {
			...
			queryString
					.append("SELECT qw.name, temp.qa_worksheet_id, temp.user_id, CONCAT(user.first_name,' ',user.last_name), "
			...
			queryString
					.append(" FROM qa_productivity_sampling "
							...
							+ "cross JOIN qa_worksheet ON qa_worksheet.id = qa_productivity_sampling.qa_worksheet_id "
							+ "cross JOIN qc_point_checklist ON qc_point_checklist.qa_productivity_sampling_id = qa_productivity_sampling.id "
							+ "cross JOIN qa_worksheet_patient_info ON qa_worksheet_patient_info.qa_productivity_sampling_id = qa_productivity_sampling.id "
							+ "WHERE 1 = 1 ");
			...
			queryString
					.append("left JOIN qa_worksheet qw ON temp.qa_worksheet_id = qw.id "
			...
			if (qaWorksheetType == Constants.QA_WORKSEET_TYPE_BYSTAFF) {
				queryString
						.append("SELECT CONCAT("
								...
								+ " ), COUNT(*), qw.status "
								+ "FROM qa_productivity_sampling AS sample "
								+ "JOIN " + entityName + " "
								+ alias + " "
								+ " JOIN qa_worksheet qw on sample.qa_worksheet_id = qw.id" + " JOIN user u on u.id = "+alias+".created_by");
				queryString.append(" WHERE 1 = 1 ");
				queryString.append("AND sample.qa_worksheet_id = "
						+ whereClause.get(Constants.QAWORKSHEET_ID) + " ");
				queryString.append("GROUP BY " + alias + ".created_by");

			} else if (qaWorksheetType == Constants.QA_WORKSEET_TYPE_BYDOCTOR) {
				queryString.append("SELECT " + doctorField
						+ " as name, COUNT(*) "
						+ "FROM qa_productivity_sampling AS sample "
						+ "JOIN " + entityName + " " + alias + " ");
				queryString.append("WHERE 1 = 1 ");
				queryString.append("AND sample.qa_worksheet_id = "
						+ whereClause.get(Constants.QAWORKSHEET_ID) + " ");
